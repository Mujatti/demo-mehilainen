...
## Mehiläinen demo variants

This package now includes two customer-tailored demo options:

### 1) Branded search-first demo
- URL: `/?config=mehilainen-brand`
- Purpose: keep the starter search experience, but restyle it to feel closer to Mehiläinen's public website.
- Changes: green healthcare palette, Meliva-style font stack, softer surfaces, pill CTAs, Mehiläinen wordmark.

### 2) Care navigator variant
- URL: `/care?config=mehilainen-care`
- Purpose: a healthcare-specific guided assistant that helps users clarify needs before booking.
- Changes: side guidance panel, care-journey quick prompts, booking support card, conversation-first workflow.

### Notes
- Styling is based on the public site plus the screenshots provided in the chat.
- The care navigator variant is intentionally schema-light, so it stays inside current architecture boundaries and does not require changes to the protected services layer.
- If you later want a doctor-comparison version, the next step is to map structured provider fields from the Mehiläinen index into a dedicated comparison card component.
